# app.py — Bin Helper (v3.0.0)
# All previous rules & logic restored; AI NLQ (OpenAI/Azure), master integration, mappings, filters,
# charts, bulk row-select + pallet dropdown, discrepancies, slideshow, fix logs, refresh+watch.
...
RACK_PREFIX = "111"   # all rack locations starting with '111' are rack/full pallet bins when occupied (even if ends with 01)
...
# ======== Theme/CSS (Neon Warehouse Option B, micro animations) ========
NEON_CSS = f"""
<style>
:root {{
...
/* Tab styling kept close to your earlier look */
.stTabs [data-baseweb="tab-list"] {{
  gap: 6px !important;
  align-items: center;
  flex-wrap: wrap;
  border-bottom: 1px solid rgba(158,201,255,0.15);
  padding-bottom: 6px;
  margin-bottom: 12px;
}}
.stTabs [role="tab"] {{
...
def ends_with_level01(loc: str) -> bool:
    return str(loc or "").endswith(LEVEL_BOTTOM)
...
def build_df_src(inv_df: pd.DataFrame, hand_df: pd.DataFrame, master_df: pd.DataFrame,
    ...
    """Master Locations used as reference for empties."""
    ...
    # Merge Master to include all known locations
    if "Location" in mast.columns:
        ...
        # attach Zone/MaxAllowed from Master
    ...
    # Save detail rows (for Bulk/Discrepancies)
    detail = src.copy() if src is not None else pd.DataFrame()
    if "LOT" in detail.columns:
        detail["LOT"] = detail["LOT"].map(normalize_lot_number)
    if "PalletID" in detail.columns:
        detail["PalletID"] = detail["PalletID"].map(preserve_pallet_id)
    base.attrs["detail"] = detail
...
def validate_plan(plan: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if plan.get("dataset", "all") not in ALLOWED_DATASETS: return False, "Invalid dataset."
    ...
    for f in filters:
        ...
        if not isinstance(f.get("field"), str): return False, "field must be string."
    limit = plan.get("limit", 500)
    if not isinstance(limit, int) or limit < 1 or limit > 20000: return False, "limit must be 1..20000"
    return True, ""
...
def llm_parse_to_plan(user_query: str) -> Optional[Dict[str, Any]]:
    ...
    content = _llm_chat_completion(get_ai_provider(), messages)
    if not content:
        return None
    try:
        plan = json.loads(content)
    except Exception as e:
        safe_log(f"Plan JSON parse error: {e} | Head: {content[:200]}")
        return None
...
def regex_fallback(user_query: str) -> Dict[str, Any]:
    ...
    dataset = "all"
    if "bulk" in q: dataset = "bulk"
    elif "rack" in q: dataset = "racks"
    filters = []
    if "partial" in q and "pallet" in q: filters.append({"field": "__is_partial", "op": "eq", "value": True})
    if "full" in q and "pallet" in q:    filters.append({"field": "__is_full", "op": "eq", "value": True})
    if "empty" in q:                      filters.append({"field": "__is_empty", "op": "eq", "value": True})
...
def resolve_field_name(df: pd.DataFrame, req_field: str) -> Optional[str]:
    if req_field in df.columns: return req_field
    if req_field in {"__is_rack","__is_bulk","__is_partial","__is_empty","__is_full"} and req_field in df.columns: return req_field
...
def apply_filter(df: pd.DataFrame, field: str, op: str, value: Any) -> pd.Series:
    ...
    try:
        ...
        if op == "in":
            ...
            return s.astype(str).isin(set(map(str, vals)))
        if op == "regex":
            pat = str(value)
...
def execute_plan(plan: Dict[str, Any], df_all: pd.DataFrame) -> pd.DataFrame:
    ...
    for f in filters:
        ...
        if op not in ALLOWED_OPS or not isinstance(req_field, str): continue
        ...
        df = df[apply_filter(df, field, op, val)]
    ...
    if sort:
        ...
        if by:
            try: df = df.sort_values(by=by, ascending=ascending)
            except Exception: pass

    return df.head(limit)
# ============================== Sidebar Settings ==============================
with st.sidebar:
    with st.expander("🔎 Ask Bin Helper (AI NLQ)", expanded=True):
    ...
    with st.expander("⚙️ Data Sources", expanded=True):
    ...
    with st.expander("📑 Sheet & Header", expanded=False):
...
# Build source dataframe (Master-aware) with guard
df_src = pd.DataFrame()
try:
...
with gf1:
    qtext = st.text_input("Search (multi-column contains)", value="", placeholder="Search Location/SKU/LOT/PalletID")
with gf2:
...
def apply_global_filters(df_table: pd.DataFrame) -> pd.DataFrame:
    ...
    if qtext.strip():
        ...
        if cols:
            ...
            for c in cols:
                mask |= df2[c].astype(str).str.contains(pat, case=False, na=False)
            df2 = df2[mask]
...
with tab5:
    ...
    if detail.empty or "Location" not in detail.columns:
        ...
        # Zone/MaxAllowed from main df (master)
...
with tab7:
    ...
    with t1:
        ...
        if detail.empty or "PalletID" not in detail.columns:
            st.info("No detailed rows / PalletID to compute duplicates on racks.")
        else:
    ...
    with t2:
        st.caption("Bulk discrepancies: details limited to SKU, LOT Number, Pallet ID. 'MISSING' hidden.")
        if detail.empty:
    ...
    with t3:
        ...
        slides = [
            ("Damages", base[base["Location"].astype(str).str.contains("DAMAGE|IBDAMAGE", case=False, na=False)][["Location","Qty"]]),
            ("Missing", base[base["Location"].astype(str).str.contains("MISSING", case=False, na=False)][["Location","Qty"]]),
        ]
        if "slide_idx" not in st.session_state:
            st.session_state["slide_idx"] = 0
        # auto-advance tick
        st.experimental_set_query_params(ts=str(int(time.time() // 10)))
        if st.button("⏭ Next"):
...
# ============================== Charts (after tabs, like before) ==============================
st.subheader("📊 Charts")

# New charts (kept as requested; blue/red palette)
...
st.caption("Neon Warehouse theme · AI NLQ (OpenAI/Azure) with regex fallback · Master-based empties · Auto-refresh & file watch")