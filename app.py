...
if tab == "Bulk Locations":
    ...
    if empty_bins_view_df.empty:
        st.info("No data available for Empty Bins.")
    else:
        st.dataframe(empty_bins_view_df)

elif tab == "Full Pallet Bins":
    st.subheader("🟩 Full Pallet Bins")
    filtered_df = apply_filters(full_pallet_bins_df)
    if filtered_df.empty:
        st.info("No data available for Full Pallet Bins.")
    else:
    ...
    if empty_partial_bins_df.empty:
        st.info("No data available for Empty Partial Bins.")
    else:
        st.dataframe(empty_partial_bins_df)

elif tab == "Partial Bins":
    st.subheader("🟥 Partial Bins")
    filtered_df = apply_filters(partial_bins_df)
    if filtered_df.empty:
        st.info("No data available for Partial Bins.")
    else:
        st.dataframe(filtered_df)

elif tab == "Damages":
    st.subheader("🛠️ Damages")
    filtered_df = apply_filters(damage_df)
    if filtered_df.empty:
        st.info("No data available for Damages.")
    else:
        st.dataframe(filtered_df)

elif tab == "Missing":
    st.subheader("❓ Missing")
    filtered_df = apply_filters(missing_df)
    if filtered_df.empty:
        st.info("No data available for Missing.")
    else:
        st.dataframe(filtered_df)
