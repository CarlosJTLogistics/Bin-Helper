...
slot_ranges = {k: 64 for k in bulk_rules}
        ...
        with st.expander(f"📍 Location: {loc} — {len(loc_issues)} issue(s)"):
            for idx, row in loc_issues.iterrows():
                ...
                for _, drow in details.iterrows():
                    st.write(f"• SKU: `{drow.get('WarehouseSku','')}` | Pallet ID: `{drow.get('PalletId','')}` | Lot: `{drow.get('CustomerLotReference','')}` | Qty: `{drow.get('Qty','')}`")
                ...
                if st.button(f"✔ Mark as Corrected: {issue}", key=f"btn_{loc}_{idx}"):
                    for _, drow in details.iterrows():
                        log_correction(loc, issue, drow.get("WarehouseSku",""), drow.get("PalletId",""), drow.get("CustomerLotReference",""), drow.get("Qty",""), notes)
